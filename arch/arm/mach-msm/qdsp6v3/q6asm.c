...
static int32_t q6asm_callback(struct apr_client_data *data, void *priv)
{
	...
	if (ac->session <= 0 || ac->session > 8) {
		pr_aud_err("%s:Session ID is invalid, session = %d\n", __func__,
			ac->session);
		return -EINVAL;
	}
			...
			uint32_t sbr_ps_enable)
{
	...
	if (rc < 0) {
		pr_aud_err("Command opcode[0x%x]paramid[0x%x] failed\n",
				ASM_STREAM_CMD_SET_ENCDEC_PARAM,
				ASM_ENABLE_SBR_PS);
		rc = -EINVAL;
		goto fail_cmd;
	...
	rc = wait_event_timeout(ac->cmd_wait,
			(atomic_read(&ac->cmd_state) == 0), 5*HZ);
	if (!rc) {
		pr_aud_err("timeout opcode[0x%x] ", sbrps.hdr.opcode);
		goto fail_cmd;
	}
	return 0;
fail_cmd:
				...
				struct asm_aac_cfg *cfg)
{
	...
	pr_aud_info("%s:format=%x cfg_size=%d aac-cfg=%x aot=%d ch=%d sr=%d\n",
				...
				uint32_t bufsz, uint32_t bufcnt)
{
	...
	if (mmap_region_cmd == NULL) {
		pr_aud_info("mmap_region_cmd == NULL\n");
		return -EINVAL;
	}
				...
				uint32_t bufsz, uint32_t bufcnt)
{
	...
	if (unmap_region_cmd == NULL) {
		pr_aud_info("unmap_region_cmd == NULL\n");
		return -EINVAL;
	}
			...
			struct asm_softpause_params *pause_param)
{
	...
	rc = apr_send_pkt(ac->apr, (uint32_t *) vol_cmd);
	if (rc < 0) {
		pr_aud_err("%s: Volume Command(soft_pause) failed\n", __func__);
		rc = -EINVAL;
		goto fail_cmd;
	}
	...
	rc = wait_event_timeout(ac->cmd_wait,
			(atomic_read(&ac->cmd_state) == 0), 5*HZ);
	if (!rc) {
		pr_aud_err("%s: timeout in sending volume command(soft_pause)"
		       "to apr\n", __func__);
		rc = -EINVAL;
		goto fail_cmd;
	}
...
uint64_t q6asm_get_session_time(struct audio_client *ac)
{
	...
	if (!rc) {
		pr_aud_err("%s: timeout in getting session time from DSP\n",
			__func__);
		goto fail_cmd;
	}
